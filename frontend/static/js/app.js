...
// Collapsible explanation toggles (each header toggles only its own section)
document.addEventListener("click", function (e) {
  if (e.target.closest("#rules-collapsible-header")) {
    const rulesSection = document.getElementById("rules-collapsible");
    const llmSection = document.getElementById("llm-collapsible");
    // Toggle rules section
    const wasOpen = rulesSection.classList.contains("open");
    rulesSection.classList.toggle("open");
    // Always close llm section
    llmSection.classList.remove("open");
    // If it was open, now close, so nothing is open
  }
  if (e.target.closest("#llm-collapsible-header")) {
    const llmSection = document.getElementById("llm-collapsible");
    const rulesSection = document.getElementById("rules-collapsible");
    // Toggle llm section
    const wasOpen = llmSection.classList.contains("open");
    llmSection.classList.toggle("open");
    // Always close rules section
...
function displayResults(result) {
  ...
  const resultLlmExplanation = document.getElementById(
  ...
  if (result.success) {
    ...
    // LLM Explanation (Gemini output formatting)
    if (result.llm_explanation && typeof result.llm_explanation === "string") {
      resultLlmExplanation.innerHTML = formatLlmExplanation(
        result.llm_explanation
      );
    } else {
      resultLlmExplanation.textContent = "-";
    }
  } else {
    // No diagnosis found
    ...
    resultMessage.textContent =
      result.message ||
      "We could not determine a diagnosis based on your symptoms.";
...
function formatLlmExplanation(text) {
  if (!text) return "-";

  // Bold for **text**
  let html = text.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>");

  // Italic for *text* (but not inside <strong>)
  ...
  lines.forEach((line) => {
    ...
    if (/^\d+\. /.test(trimmed)) {
      ...
      formatted += `<li>${trimmed.replace(/^\d+\. /, "")}</li>`;
